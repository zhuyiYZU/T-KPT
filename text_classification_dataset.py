    ...
    def get_examples(self, data_dir, split):
        ...
        with open(path, encoding='utf-8')as f:
            ...
            for idx, line in enumerate(lines[1:]):
                ...
                examples.append(example)
        return examples
class Toxi2Processor(DataProcessor):
...
class CNIProcessor(DataProcessor):
...
class CNIcontentProcessor(DataProcessor):
    ...
    def get_examples(self, data_dir, split):
        ...
        with open(path, encoding='utf-8')as f:
            reader = csv.reader(f,delimiter=',')
            for idx, row in enumerate(reader):
                label, headline, content = row
                text_a = headline.replace('\\',' ')
                text_b = content.replace('\\', ' ')
                #print(text_a)
                #label = int(label)
...
class ProsProcessor(DataProcessor):
...
class THUProcessor(DataProcessor):
    ...
    def __init__(self):
        super().__init__()
        self.labels = ['财经', '彩票','房产','股票','家居','教育','科技','社会','时尚','时政','体育','星座','游戏','娱乐']
...
PROCESSORS = {
    ...
    "ToxiCN":Toxi2Processor,
    "chinese_implict":CNIProcessor,
    "chinese_implict_content":CNIcontentProcessor,
    "ProsCons":ProsProcessor,
    "THUCNews":THUProcessor,
    "ChnSentiCorp":ChnProcessor,
}
